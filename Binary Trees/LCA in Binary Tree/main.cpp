...
class Solution {
  private:
    ...
    void dfs(Node* root,bool& foundN1, bool& foundN2) {
        if(!root) return;
        
        if(root->data == n1) foundN1 = true;
        if(root->data == n2) foundN2 = true;
        
        if(root->left) dfs(root->left,foundN1,foundN2);
        if(root->right) dfs(root->right,foundN1,foundN2);
  ...
  public:
    ...
    void findAncestor(Node* root) {
        
        bool foundN1 = false;
        bool foundN2 = false;
        dfs(root,foundN1,foundN2);
        
        if(foundN1 && foundN2) ancestor = root;
        else {
            return;
        }
        if(root->left) findAncestor(root->left);
        if(root->right) findAncestor(root->right);
    }
